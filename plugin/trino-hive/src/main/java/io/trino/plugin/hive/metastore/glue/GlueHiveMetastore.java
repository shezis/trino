        ...
        implements HiveMetastore
{
    ...
    {
        return getTableStream(databaseName)
    ...
    public Map<String, RelationType> getRelationTypes(String databaseName)
    {
        return getTableStream(databaseName)
                .collect(toImmutableMap(
                        software.amazon.awssdk.services.glue.model.Table::name,
                        table -> {
                            // GlueHiveMetastore currently does not distinguish views and materialized views, see getAllViews().
                            if (VIRTUAL_VIEW.name().equals(table.tableType())) {
    ...
    {
        // parameterValue.equals(getTableParameters(table).get(parameterKey))
        return getTableStream(databaseName)
                .filter(table -> parameterValue.equals(table.parameters().get(parameterKey)))
                .map(software.amazon.awssdk.services.glue.model.Table::name)
                .toList();
    }
    ...
    private Stream<software.amazon.awssdk.services.glue.model.Table> getTableStream(String databaseName)
    {
        return stats.getGetTables().call(() -> glueClient.getTablesPaginator(builder -> builder
                        .applyMutation(glueContext::configureClient)
    ...
    public void updateTableStatistics(String databaseName, String tableName, AcidTransaction transaction, StatisticsUpdateMode mode, PartitionStatistics statisticsUpdate)
    {
        ...
        // existing column statistics are required for the columns being update when in merge incremental mode
    ...
    public Map<PartitionName, Optional<Partition>> getPartitionsByNames(String databaseName, String tableName, Collection<PartitionName> partitionNames)
    {
            ...
            while (!pendingPartitions.isEmpty()) {
                ...
                for (var response : responses) {
                    ...
                    if (response.partitions().isEmpty()) {
                        verify(!response.unprocessedKeys().isEmpty(), "Empty unprocessedKeys for non-empty BatchGetPartitionRequest and empty partitions result");
